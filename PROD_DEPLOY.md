...
Это делает имена volumes/network **детерминированными** вне зависимости от cwd/обёрток.

Почему не используем верхнеуровневое `name:`:
- production-сервер работает на legacy `docker-compose` v1.29.2, который не поддерживает top-level `name:`.
- Ранее идея с `name: dedato` была протестирована, но откатана ради совместимости.
...
- Убедитесь, что используется **только** `docker-compose.prod.yml` из репозитория (с явными `volume.name` и `networks.default.name`).
