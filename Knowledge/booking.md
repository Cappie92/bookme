...
Канон текущего repository-known жизненного цикла записи. Расчёт доступности принадлежит [Scheduling](scheduling.md), HTTP-границы — [booking API contract](booking-api.md), подтверждённые ограничения — [booking and scheduling debt](booking-scheduling.md).
...
**Source:** `backend/models.py` — `BookingConfirmation`, `Income`, `TemporaryBooking`; `backend/services/scheduling.py`; create routers; [completion side effects](booking-completion-side-effects.md).
