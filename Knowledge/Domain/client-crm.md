...
Master client-list/card handlers проверяют `has_clients_access`. Общие master restriction handlers имеют authentication/owner checks, но не тот же capability guard; это зафиксировано в [feature entitlement Debt](../Debt/feature-entitlements-and-jobs.md#entitlement-enforcement-is-not-centralized).
...
Failure boundaries и schema/lifecycle drift находятся в [CRM/Loyalty/Promo/Finance Debt](../Debt/client-crm-loyalty-promo-finance.md). Никакой event bus между Booking и CRM не подтверждён: агрегаты читают Booking напрямую, а restriction checks вызываются синхронно.
