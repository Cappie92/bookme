...
OAuth onboarding requires terms and personal-data booleans before account creation, accepts a marketing choice, and does not persist any of those choices with text/version/time evidence. Therefore current DB cannot prove consent history for either common or OAuth registration. This is [high privacy Debt](../Debt/security-and-privacy.md#high-registration-consent-evidence), not a claim that no legal basis exists.
...
Registration, mobile auth diagnostics and call-verification service contain log statements that can include contact attributes or credential/provider response structures. Production reach, sinks and retention are UNKNOWN. Knowledge does not reproduce those values; the confirmed exposure surface is tracked in [sensitive logging Debt](../Debt/security-and-privacy.md#high-sensitive-logging-surfaces).
