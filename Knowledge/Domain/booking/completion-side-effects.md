...
Single confirm требует past start; status update вызывает service с `require_past_start=False`, а endpoint-specific guards определяют допустимость. Auto-to-manual compatibility helper не вызывает service и описан в [Debt](../../Debt/booking-scheduling.md).
