...
Object ownership и business/capability guards не выводятся из bearer автоматически: их добавляет router/service. Конкретный authorization contract описан в [Identity API](identity-api.md), [Identity and access](identity-access.md) и domain Debt.
...
**Sources:** `backend/database.py`; transaction call sites; [Booking completion](booking-completion-side-effects.md); [Promo](promo.md); [Billing invariants](subscriptions-billing-invariants.md).
...
Подтверждённые gaps находятся в [Backend/API Debt](backend-api.md).
