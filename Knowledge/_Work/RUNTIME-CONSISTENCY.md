...
Scoped Identity/Authorization follow-up checked remediation commit `e0b8bc7`; verdicts for unrelated packages were not changed.
...
| `Knowledge/Contracts/identity-api.md` | Identity HTTP contract | CURRENT | Self-service role allowlist, admin role enforcement, registration/OAuth/login/refresh, contact changes and account deletion | None found after RC-001 remediation | Provider behavior and token use outside repository |
...
| `Knowledge/Debt/booking-scheduling.md` | Booking/scheduling debt | CURRENT | Resolved generic object authorization plus remaining overlap/status/completion risks | None found after scoped remediation sync | Production occurrence and affected records |
...
| `Knowledge/Debt/security-and-privacy.md` | Sanitized security/privacy debt | CURRENT | Resolved registration/admin boundaries plus remaining credential, verification, logging and data-discovery Debt | No sensitive value or validity claim was re-evaluated | Credential validity, provider state, log retention |
...
| `Knowledge/Domain/identity-access.md` | Identity and actual authorization enforcement | CURRENT | Self-service roles, admin/moderator guard composition, Booking resource scope, session/token and account lifecycle | None found after RC-001 remediation | Live provider modes and issued tokens |
