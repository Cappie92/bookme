...
Admin plan writes validate that referenced `ServiceFunction` rows exist, are active and are FREE/SUBSCRIPTION type at write time. Runtime checks use the numeric list directly and do not re-read current `ServiceFunction.is_active`; see [Debt](feature-entitlements-and-jobs.md#service-function-identity-and-activation-drift).
...
This table is factual runtime behavior. Missing checks are not intended policy; confirmed gaps are in [Debt](feature-entitlements-and-jobs.md).
