...
**Source:** `backend/main.py` — startup/shutdown; `backend/Dockerfile`; `docker-compose.prod.yml`; [Production topology](production-topology.md#startup-and-process-model).
...
Startup catch-up walks active subscription date ranges from the last successful charge (or start date) through today and calls the same charge function. The subsequent current-date pass encounters the row just created by catch-up: the charge function refuses a duplicate, but the aggregate pass classifies that response as a failed charge. This observability drift is tracked in [Debt](feature-entitlements-and-jobs.md#daily-catch-up-result-drift). The daily pass also marks elapsed active subscriptions expired, activates due pending subscriptions, records failures and deactivates subscriptions when the charge path reports insufficient funds.

Application-level idempotency queries `DailySubscriptionCharge` by subscription/date before mutation. The DB index on that pair is not unique, so concurrent processes can pass the pre-check; this is [Debt](feature-entitlements-and-jobs.md#daily-charge-concurrency).

Autorenewal check does not initiate recurring card payments; it records the current not-implemented outcome. Full money semantics remain in [Subscriptions billing](subscriptions-billing.md).
...
**Source:** `backend/services/recurring_expenses.py`; `backend/models.py` — `MasterExpense`; [Debt](feature-entitlements-and-jobs.md#recurring-expense-recovery-and-idempotency).
...
**Source:** `backend/services/bookings_limit_monitor.py`; `backend/utils/master_future_bookings_query.py`; [Feature entitlements](feature-entitlements.md#6-limits).
...
**Source:** service implementations; `backend/main.py` — `/health`; [Debt](feature-entitlements-and-jobs.md#job-health-and-event-loop-coupling).
...
Confirmed failure scenarios and remediation boundaries: [Feature entitlement and job Debt](feature-entitlements-and-jobs.md).
