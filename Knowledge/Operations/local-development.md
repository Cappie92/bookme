...
The root `make backend-run` wrapper starts the same entrypoint on loopback using the caller's active `python3`; activate the intended environment first if using that wrapper. Configuration precedence and safe environment rules belong to [Configuration](../Infrastructure/configuration.md). Schema lifecycle belongs to [Data and migrations](../Architecture/data-and-migrations.md); application `create_all` is not a substitute for reviewing/applying migrations.
...
Use the package platform scripts only after local Expo/native prerequisites are installed. API/WEB URL precedence and production URL rejection are defined in [Mobile architecture](../Architecture/mobile.md) and [Configuration](../Infrastructure/configuration.md). Do not copy values from credential-like repository artifacts into local config.
...
**Sources:** `docker-compose.yml` path/structure only; [Security and privacy Debt](../Debt/security-and-privacy.md).
