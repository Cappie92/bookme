...
Production Dockerfile запускает один Uvicorn process без explicit worker count. Фактическая production topology, migration ordering и host state принадлежат [Production topology](production-topology.md) и [Data and migrations](data-and-migrations.md).
...
Общие path, validation, serialization, auth/error и compatibility правила находятся в [API conventions](api-conventions.md). Подтверждённый architectural drift — в [Backend/API Debt](backend-api.md).
