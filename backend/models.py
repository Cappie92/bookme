...
class Master(Base):
    ...
    address = Column(String, nullable=True)  # Адрес мастера (для карты и публичной страницы)
    address_detail = Column(Text, nullable=True)  # Уточнение: этаж, подъезд, домофон (не в ссылку на карты)
    ...
    timezone = Column(String, nullable=True)  # Часовой пояс; без дефолта — принудительный выбор после регистрации
    timezone_confirmed = Column(Boolean, default=False, nullable=False)  # True после явного выбора города+таймзоны
    ...
    client_restrictions = relationship("ClientRestriction", back_populates="master")
    payment_settings = relationship("MasterPaymentSettings", back_populates="master", uselist=False)
    loyalty_discounts = relationship("LoyaltyDiscount", back_populates="master")
    personal_discounts = relationship("PersonalDiscount", back_populates="master")
    client_metadata = relationship("MasterClientMetadata", back_populates="master")


class MasterClientMetadata(Base):
    """Метаданные мастер→клиент: alias (отображаемое имя), заметка (max 280)."""
    __tablename__ = "master_client_metadata"
    ...
    master = relationship("Master", back_populates="client_metadata")

    __table_args__ = (
        Index("idx_master_client_metadata_master", "master_id"),
        Index("idx_master_client_metadata_phone", "client_phone"),
        UniqueConstraint("master_id", "client_phone", name="uq_master_client_metadata_master_phone"),
    )
...
class IndieMaster(Base):
    ...
    master = relationship("Master", backref="indie_master_profile")
...
class BookingStatus(str, enum.Enum):
    ...
    CONFIRMED = "confirmed"  # мастер принял запись (pre-visit)
    AWAITING_CONFIRMATION = "awaiting_confirmation"  # прошлая запись, ждёт outcome
    COMPLETED = "completed"  # услуга состоялась
    CANCELLED = "cancelled"
...
class Booking(Base):
    ...
    # Публичный код для клиента (не sequential id); уникален, задаётся при INSERT
    public_reference = Column(String(20), unique=True, index=True, nullable=False)
...
class Subscription(Base):
    ...
    apple_product_id = Column(String(128), nullable=True, index=True)
    apple_environment = Column(String(32), nullable=True)  # sandbox | production
...
class SubscriptionReservation(Base):
    ...
    reserved_amount = Column(Float, nullable=False, default=0.0)  # Зарезервированная сумма в рублях
...
class SubscriptionPriceSnapshot(Base):
    ...
    # Idempotency для бесплатного apply (final_price<=0) и для диагностики
    applied_subscription_id = Column(Integer, ForeignKey("subscriptions.id"), nullable=True)
...
class BalanceTransaction(Base):
    ...
    balance_before = Column(Float, nullable=False)  # Баланс до операции в рублях
    balance_after = Column(Float, nullable=False)   # Баланс после операции в рублях
...
class DailySubscriptionCharge(Base):
    ...
    # Сумма списания за день (в рублях)
    amount = Column(Float, nullable=False)
    
    # Стоимость дня по тарифу (в рублях)
    ...
    # Причина (для FAILED: например "daily_rate > balance")
...
class LoyaltyDiscount(Base):
    ...
    salon_id = Column(Integer, ForeignKey("salons.id"), nullable=True)  # Legacy, для обратной совместимости
    ...
    # Связи
    master = relationship("Master", back_populates="loyalty_discounts")
    salon = relationship("Salon", overlaps="loyalty_discounts")  # Legacy
    
    # Индексы
    __table_args__ = (
        Index("idx_loyalty_discount_master", "master_id"),
        Index("idx_loyalty_discount_salon", "salon_id"),  # Legacy
...
class PersonalDiscount(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    master_id = Column(Integer, ForeignKey("masters.id"), nullable=True, index=True)  # Nullable для миграции, затем NOT NULL
    salon_id = Column(Integer, ForeignKey("salons.id"), nullable=True)  # Legacy, для обратной совместимости
    ...
    # Связи
    master = relationship("Master", back_populates="personal_discounts")
    salon = relationship("Salon", overlaps="personal_discounts")  # Legacy
    
    # Индексы
    __table_args__ = (
        Index("idx_personal_discount_master", "master_id"),
        Index("idx_personal_discount_salon", "salon_id"),  # Legacy
...
class ClientRestriction(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    salon_id = Column(Integer, ForeignKey("salons.id"), nullable=True)  # ID салона (null для мастеров)
    indie_master_id = Column(Integer, ForeignKey("indie_masters.id"), nullable=True)  # Legacy: indie_masters.id
    master_id = Column(Integer, ForeignKey("masters.id"), nullable=True)  # Master-only: masters.id
    client_phone = Column(String, nullable=False)  # Номер телефона клиента
    ...
    __table_args__ = (
        ...
        Index("idx_client_restriction_master", "master_id"),
...
class TemporaryBooking(Base):
    ...
    Скидка фиксируется при создании (fixed_discount_*) и не пересчитывается при confirm.
    """
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    # Фиксированная скидка при создании (не пересчитывается при confirm)
    fixed_discount_rule_type = Column(String, nullable=True)  # 'loyalty' | 'personal'
    fixed_discount_rule_id = Column(Integer, nullable=True)
    fixed_discount_percent = Column(Float, nullable=True)
    fixed_discount_amount = Column(Float, nullable=True)
...
class AdminOperation(Base):
    ...
    # Сумма операции (в рублях)
    amount_rubles = Column(Float, nullable=False)
...
class SubscriptionPlan(Base):
    ...
    freeze_days_1month = Column(Integer, nullable=True, default=0, server_default='0')  # Дни заморозки для подписки на 1 месяц
    freeze_days_3months = Column(Integer, nullable=True, default=0, server_default='0')  # Дни заморозки для подписки на 3 месяца
    freeze_days_6months = Column(Integer, nullable=True, default=0, server_default='0')  # Дни заморозки для подписки на 6 месяцев
    freeze_days_12months = Column(Integer, nullable=True, default=0, server_default='0')  # Дни заморозки для подписки на 12 месяцев
...
class Payment(Base):
    """Модель платежей через Robokassa"""
    ...
    # Статус применения подписки (для безопасного ретрая apply после paid)
...
class GlobalSettings(Base):
    """Глобальные настройки приложения (фича-флаги, конфигурация)"""
    ...
    # Значение настройки (JSON для гибкости)
...
from utils.booking_public_reference import ensure_booking_public_reference_allocated  # noqa: E402
