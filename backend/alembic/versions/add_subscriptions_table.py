...
def upgrade():
    ...
    index_defs = (
        ('idx_subscription_user_type', ['user_id', 'subscription_type']),
        ('idx_subscription_status', ['status']),
        ('idx_subscription_valid_until', ['valid_until']),
        (op.f('ix_subscriptions_id'), ['id']),
    )
    for name, cols in index_defs:
        if not all(c in sub_cols for c in cols):
            continue
        insp = sa.inspect(bind)
        idx_names = {ix['name'] for ix in insp.get_indexes('subscriptions')}
        if name not in idx_names:
            op.create_index(name, 'subscriptions', cols, unique=False)
