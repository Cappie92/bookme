...
class Booking(BaseModel):
    ...
    applied_discount: Optional[AppliedDiscountInfo] = None
...
class BlogPostImportFrontmatter(BaseModel):
    """Подмножество полей статьи из YAML frontmatter (импорт .md, MVP)."""
    ...
    def normalize_tags(cls, v: Any) -> List[str]:
        if v is None:
            return []
        if isinstance(v, str):
            parts = [t.strip() for t in v.replace(";", ",").split(",")]
            return [t for t in parts if t]
        if isinstance(v, list):
            return [str(x).strip() for x in v if str(x).strip()]
        raise ValueError("tags должен быть списком строк или одной строкой через запятую")

    @field_validator("robots_noindex", "robots_nofollow", mode="before")
    @classmethod
    def coerce_bool(cls, v: Any) -> bool:
        ...
        if isinstance(v, str):
            low = v.lower().strip()
            if low in ("true", "yes", "1", "on", "да"):
                return True
            if low in ("false", "no", "0", "off", "нет"):
                return False
        if isinstance(v, int):
            return v != 0
        return bool(v)

    @field_validator("published_at", "scheduled_at", mode="before")
    @classmethod
    def parse_optional_datetime(cls, v: Any) -> Optional[datetime]:
        if v is None or v == "":
            return None
        ...
        if isinstance(v, str):
            ...
            try:
                ...
                raise ValueError("ожидается дата-время в формате ISO 8601") from e
        raise ValueError("некорректный тип даты")


class BlogPostImportPreviewResponse(BaseModel):
    """Ответ POST /admin/blog/import/preview (без записи в БД)."""
...
# Reusable schemas для OpenAPI (ошибки и простые ответы)
class HTTPErrorDetail(BaseModel):
    """Стандартный формат ошибки API (detail от FastAPI/HTTPException)."""
    detail: str


class MessageOut(BaseModel):
    """Ответ с одним текстовым сообщением."""
    message: str


class PublicBookingCreateOut(BaseModel):
    """Ответ создания брони с публичной страницы мастера."""
    id: int
    status: str
...
class AvailableSlotOut(BaseModel):
    """Один доступный слот для бронирования (start/end — datetime в API)."""
    start_time: datetime
    end_time: datetime


class DomainSubdomainInfoOut(BaseModel):
    """Информация о владельце поддомена (салон / indie мастер / мастер). Все поля кроме owner_type и owner_id опциональны."""
    owner_type: str
    owner_id: int
    name: Optional[str] = None
    description: Optional[str] = None
    ...
    class Config:
        extra = "allow"  # допустить доп. поля из ответа (разные типы владельцев)
...
# MASTER_CANON: без indie_master_id (жёсткий контракт master-only)
...
class MasterDayOpenSlotIn(BaseModel):
    """Один открытый 30-мин слот на конкретную дату (как в weekly API)."""

    hour: int = Field(..., ge=0, le=23)
    minute: int = Field(..., ge=0, le=59)


class MasterDayScheduleUpdate(BaseModel):
    """
    Локальная замена открытых слотов MasterSchedule только на одну дату.
    Не изменяет MasterScheduleSettings (recurring rule).
    """

    schedule_date: date = Field(..., description="Календарная дата YYYY-MM-DD")
    open_slots: List[MasterDayOpenSlotIn] = Field(
        default_factory=list,
        description="Явно открытые 30-мин интервалы; отсутствующие в списке — закрыты",
    )
...
class SubscriptionCalculationResponse(BaseModel):
    ...
    # --- Breakdown для UI/отладки (особенно для immediate upgrade) ---
    # Сколько "кредита" от текущей активной подписки учтено в расчетах
    current_plan_credit: Optional[float] = None
    # Сколько уже "начислено" (использовано) по текущей подписке (оценка)
    current_plan_accrued: Optional[float] = None
    # Остаток резерва (hold) по текущей подписке (если есть)
    current_plan_reserved_remaining: Optional[float] = None
    # Параметры текущей подписки (если есть)
    current_plan_price: Optional[float] = None
    current_plan_daily_rate: Optional[float] = None
    # Стоимость нового тарифа без учета кредита (дублирует total_price, но явно для UI)
    new_plan_cost: Optional[float] = None
    # Итог к оплате (дублирует final_price, но явно для UI)
    payable: Optional[float] = None
    # Откуда взяли кредит: reservation | charges | none | mixed
...
class TransactionOut(BaseModel):
    ...
    amount_kopecks: float  # Оставляем для обратной совместимости, но теперь это рубли
    transaction_type: str
    description: Optional[str] = None
    subscription_id: Optional[int] = None
...
class LoyaltyDiscount(LoyaltyDiscountBase):
    id: int
    master_id: int
    salon_id: Optional[int] = None  # Legacy, для обратной совместимости
    created_at: datetime
    updated_at: datetime
...
class QuickDiscountBulkDeactivateRequest(BaseModel):
    """Массовая деактивация всех активных quick-правил одного condition_type."""
...
class DiscountEvaluationBooking(BaseModel):
    start_time: Optional[datetime] = None
    service_id: Optional[int] = None
...
class DiscountEvaluationRequest(BaseModel):
    ...
    booking: DiscountEvaluationBooking


class DiscountCandidate(BaseModel):
    rule_id: int
    rule_type: str
    name: str
...
class DiscountEvaluationResponse(BaseModel):
    ...
    best_candidate: Optional[DiscountCandidate] = None
...
class SubscriptionPlanBase(BaseModel):
    ...
    freeze_days_1month: Optional[int] = 0  # Дни заморозки для подписки на 1 месяц
    freeze_days_3months: Optional[int] = 0  # Дни заморозки для подписки на 3 месяца
    freeze_days_6months: Optional[int] = 0  # Дни заморозки для подписки на 6 месяцев
    freeze_days_12months: Optional[int] = 0  # Дни заморозки для подписки на 12 месяцев
...
class SubscriptionPlanUpdate(BaseModel):
    ...
    freeze_days_1month: Optional[int] = None
    freeze_days_3months: Optional[int] = None
    freeze_days_6months: Optional[int] = None
    freeze_days_12months: Optional[int] = None
...
class PaymentOut(BaseModel):
    ...
    is_recurring: bool
    robokassa_recurring_id: Optional[str] = None
