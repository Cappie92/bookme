...
def get_or_create_user_balance(db: Session, user_id: int, do_commit: bool = True) -> UserBalance:
    ...
    ВАЖНО: по умолчанию коммитит при создании (историческое поведение).
    Для атомарных операций используйте do_commit=False (только flush).
    """
...
def get_user_reserved_total(db: Session, user_id: int) -> float:
    """Сумма зарезервированных средств по всем подпискам пользователя (в рублях)"""
...
def move_available_to_reserve(db: Session, subscription: Subscription, amount: float, do_commit: bool = True) -> bool:
    """Переместить средства из доступного баланса в резерв подписки.

    ВАЖНО: по умолчанию коммитит. Для транзакционного apply используйте do_commit=False.
    """
    if amount <= 0:
        return True
    available = get_user_available_balance(db, subscription.user_id, do_commit=do_commit)
    if available < amount:
        return False
...
def reserve_full_subscription_price(db: Session, subscription: Subscription) -> Dict[str, Any]:
    ...
    available_balance = get_user_available_balance(db, subscription.user_id)
    
    if available_balance < full_price:
        return {
            "success": False,
    ...
    reservation.reserved_amount = full_price
...
def ensure_reserve_for_remaining_days(db: Session, subscription: Subscription, max_days: Optional[int] = None) -> Dict[str, Any]:
    ...
    target_reserve = target_days * daily_rate
    reservation = db.query(SubscriptionReservation).filter(SubscriptionReservation.subscription_id == subscription.id).first()
    current_reserved = reservation.reserved_amount if reservation else 0.0
    missing = max(0.0, target_reserve - current_reserved)
    ok = move_available_to_reserve(db, subscription, missing) if missing > 0 else True
    final_reserved = current_reserved + (missing if ok else 0.0)
    return {
...
def auto_renew_subscription(db: Session, subscription: Subscription) -> Dict[str, Any]:
    ...
    Variant A (без “кошелька” в восприятии пользователя):
    - recurring Robokassa (токенизация/ребиллы) сейчас не реализован в коде,
      поэтому мы НЕ продлеваем подписки автоматически “из available”.
    - продление делается либо вручную (оплата следующего периода -> PENDING подписка),
      либо будет реализовано позже через recurring API.
    """
    return {
        "success": False,
        "message": "Автопродление по карте (recurring) не реализовано. Используйте ручное продление (оплата следующего периода заранее).",
    }
...
def add_balance_transaction_no_commit(
    db: Session,
    user_id: int,
    amount: float,
    transaction_type: TransactionType,
    ...
    """Вариант add_balance_transaction без commit() — для атомарных транзакций."""
    user_balance = get_or_create_user_balance(db, user_id, do_commit=False)
    balance_before = user_balance.balance
    user_balance.balance += amount
    balance_after = user_balance.balance
...
def withdraw_balance(db: Session, user_id: int, amount_rubles: float, description: str = "Списание") -> Dict[str, Any]:
    ...
    transaction = add_balance_transaction(
        db=db,
        user_id=user_id,
        amount=-amount_rubles,  # Отрицательная сумма для списания
        transaction_type=TransactionType.WITHDRAWAL,
        description=description
    )
...
def calculate_upgrade_cost(
    ...
    additional_payment = max(0.0, new_cost - unused_balance)
...
def process_daily_charge(db: Session, subscription_id: int, charge_date: date = None) -> Dict[str, Any]:
    ...
    if balance_before < charge_amount:
        ...
        db.add(charge_record)
        # Деактивация: прямой UPDATE по PK для гарантированного сохранения
        db.execute(text("UPDATE subscriptions SET is_active = 0 WHERE id = :sid"), {"sid": subscription_id})
        db.flush()
        db.commit()
        from settings import get_settings
        if get_settings().DAILY_CHARGE_DEBUG.strip() == "1":
            import logging as _log
            _log.getLogger(__name__).info(
                "DAILY_CHARGE_DEBUG deactivated sub_id=%s user_id=%s balance_before=%.2f daily_rate=%.2f "
                "charge_amount=%s is_active_before=%s commit_done=True",
                subscription_id, subscription.user_id, balance_before, daily_rate,
                charge_amount, is_active_before,
            )
        return {
            "success": False,
            "error": reason,
...
def get_subscription_status(db: Session, user_id: int, subscription_type: str) -> Dict[str, Any]:
    ...
    Текущая подписка выбирается так же, как для feature gating:
    `get_user_subscription_with_plan` (ACTIVE, end_date > now, приоритет по end_date desc).
    """
    # Проверяем, является ли пользователь is_always_free
    user = db.query(User).filter(User.id == user_id).first()
    is_always_free = user.is_always_free if user else False

    from utils.subscription_features import get_user_subscription_with_plan

    try:
        st = (
            subscription_type
            if isinstance(subscription_type, SubscriptionType)
            else SubscriptionType(subscription_type)
        )
    except ValueError:
        return {
    ...
    subscription = get_user_subscription_with_plan(db, user_id, st, now_utc=now_utc)

    # Крайний случай: is_always_free, но в БД нет планов AlwaysFree/Premium —
    # get_user_subscription_with_plan вернёт None; сохраняем прежний «виртуальный» ответ.
    ...
    # MVP: резерв не используем; reserved_days = дни по депозиту (как days_remaining)
    reserved_days = days_remaining
