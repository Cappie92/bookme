...
def validate_service_functions(db: Session, service_function_ids: List[int]) -> None:
    """
    Валидировать service_functions: проверить, что все существуют, активны и имеют тип FREE или SUBSCRIPTION.
    FREE функции доступны всем планам, SUBSCRIPTION функции доступны только платным планам.
    """
    ...
    # Разрешаем FREE и SUBSCRIPTION функции в планах подписки
    # Проверяем как через enum, так и через строки (на случай если в БД хранится строка)
    allowed_types = {"FREE", "SUBSCRIPTION"}
    wrong_type_functions = [f for f in functions if f.function_type.upper() not in allowed_types]
    if wrong_type_functions:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Service functions с ID {[f.id for f in wrong_type_functions]} имеют недопустимый тип '{[f.function_type for f in wrong_type_functions]}'. Разрешены только FREE и SUBSCRIPTION"
        )
...
def create_subscription_plan(
    ...
    plan = SubscriptionPlan(
        ...
        freeze_days_1month=plan_data.freeze_days_1month or 0,
        freeze_days_3months=plan_data.freeze_days_3months or 0,
        freeze_days_6months=plan_data.freeze_days_6months or 0,
        freeze_days_12months=plan_data.freeze_days_12months or 0,
        features=features,
