...
async def deposit_balance_endpoint(
    ...
    """Пополнение отключено. MVP: депозит только через оплату подписки (Продлить/Апгрейд)."""
    raise HTTPException(
...
async def get_transaction_history(
    ...
    return [
        TransactionOut(
            id=t.id,
            amount=t.amount,
            amount_kopecks=t.amount,  # Оставляем для обратной совместимости, но теперь это рубли
