"""Apple IAP entitlement sync — mapping, idempotency, identity, daily-charge exclusion."""
...
def _rc_payload(product_id: str, *, days=30, sandbox=True, store_tx="tx-orig-1"):
    ...
    return {
        "subscriber": {
            "subscriptions": {
                product_id: {
                    "expires_date": (now + timedelta(days=days)).strftime("%Y-%m-%dT%H:%M:%SZ"),
...
def test_sync_basic_1m(db):
    ...
    with patch(
        ...
        return_value=_rc_payload("dedato_basic_1m"),
    ):
        result = sync_apple_entitlement_for_user(db, user)
    assert result["active"] is True
    assert result["plan_name"] == "Basic"
    assert result["duration_months"] == 1
    sub = db.query(Subscription).filter(Subscription.id == result["subscription_id"]).one()
    assert sub.billing_provider == "apple"
    assert sub.apple_product_id == "dedato_basic_1m"
...
@pytest.mark.parametrize(
    ...
    [
        ("dedato_basic_3m", "Basic", 3),
        ("dedato_basic_6m", "Basic", 6),
        ("dedato_basic_12m", "Basic", 12),
        ("dedato_pro_1m", "Pro", 1),
        ("dedato_premium_1m", "Premium", 1),
    ],
)
...
def test_sync_idempotent(db):
    user = _seed_master(db)
    _seed_plan(db, "Basic")
    payload = _rc_payload("dedato_basic_1m", store_tx="same-tx")
    with patch(
        "services.apple_subscription_sync.fetch_revenuecat_subscriber",
...
def test_other_user_unaffected(db):
    ...
    with patch(
        "services.apple_subscription_sync.fetch_revenuecat_subscriber",
        return_value=_rc_payload("dedato_basic_1m", store_tx="u1-tx"),
    ):
...
def test_expired_entitlement_deactivates_apple(db):
    ...
    sub = Subscription(
        ...
        apple_product_id="dedato_basic_1m",
...
def test_active_robokassa_not_deactivated_by_apple_sync(db):
    ...
    with patch(
        "services.apple_subscription_sync.fetch_revenuecat_subscriber",
        return_value=_rc_payload("dedato_basic_1m", store_tx="should-not-activate"),
    ):
        result = sync_apple_entitlement_for_user(db, user)
    ...
    assert (
        ...
        .filter(Subscription.user_id == user.id, Subscription.billing_provider == "apple")
        .count()
        == 0
    )
...
def test_apple_sync_blocked_when_robokassa_active(db):
    ...
    with patch(
        "services.apple_subscription_sync.fetch_revenuecat_subscriber",
        return_value=_rc_payload("dedato_basic_1m", store_tx="blocked-tx"),
    ):
        result = sync_apple_entitlement_for_user(db, user)
...
def test_expired_robokassa_allows_apple_sync(db):
    ...
    with patch(
        "services.apple_subscription_sync.fetch_revenuecat_subscriber",
        return_value=_rc_payload("dedato_basic_1m", store_tx="after-robo"),
    ):
        result = sync_apple_entitlement_for_user(db, user)

    assert result["active"] is True
    assert result["reason"] == "created"
    apple = db.query(Subscription).filter(Subscription.id == result["subscription_id"]).one()
...
def test_production_metadata(db):
    ...
    with patch(
        ...
        return_value=_rc_payload(
            "dedato_premium_1m", sandbox=False, store_tx="prod-tx"
        ),
    ):
        result = sync_apple_entitlement_for_user(db, user)
    sub = db.query(Subscription).filter(Subscription.id == result["subscription_id"]).one()
