"""Тесты: список клиентов показывает только completed; detail/PATCH доступны по metadata/phone."""
import pytest
...
def master_user(db):
    u = User(
        email="master_compl@test.com",
        hashed_password=get_password_hash("test123"),
        phone="+79990000008",
        full_name="Master Completed",
        role="master",
        is_active=True,
        is_verified=True,
    )
...
def client_user(db):
    u = User(
        email="client_compl@test.com",
        hashed_password=get_password_hash("test123"),
        phone="+79993000004",
        full_name="Client With Completed",
        role="client",
        is_active=True,
        is_verified=True,
    )
...
def test_client_with_completed_in_list(client, master_user, master_record, master_with_clients_plan, client_user, completed_booking, master_token):
    """Клиент с completed записью появляется в GET /api/master/clients."""
    r = client.get("/api/master/clients", headers={"Authorization": f"Bearer {master_token}"})
    assert r.status_code == 200
    rows = r.json()
    assert any(x.get("client_phone") == "+79993000004" for x in rows)


def test_search_finds_completed_client(client, master_user, master_record, master_with_clients_plan, client_user, completed_booking, master_token):
    """Поиск q= находит клиента с completed."""
    r = client.get("/api/master/clients?q=93000004", headers={"Authorization": f"Bearer {master_token}"})
    assert r.status_code == 200
    rows = r.json()
    assert any(x.get("client_phone") == "+79993000004" for x in rows)
