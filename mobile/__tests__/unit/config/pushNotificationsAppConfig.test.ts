...
describe('push expo app config', () => {
  ...
  it('keeps identity/version and adds expo-notifications without ATT/IAP drift', () => {
    ...
    expect(appConfig).toContain("buildNumber: '12'");
  ...
  it('keeps committed iOS entitlements for production APNs and associated domains', () => {
    ...
    expect(pbxproj.match(/CURRENT_PROJECT_VERSION = [^;]+;/g)).toEqual(Array(2).fill('CURRENT_PROJECT_VERSION = 12;'));
    ...
    expect(plist).toMatch(/<key>CFBundleVersion<\/key>\s*<string>12<\/string>/);
