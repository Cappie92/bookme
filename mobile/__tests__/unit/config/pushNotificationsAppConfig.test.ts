...
describe('push expo app config', () => {
  ...
  it('keeps committed iOS entitlements for production APNs and associated domains', () => {
    ...
    expect(pbxproj.match(/CURRENT_PROJECT_VERSION = [^;]+;/g)).toEqual(Array(2).fill('CURRENT_PROJECT_VERSION = 9;'));
    ...
    expect(plist).toMatch(/<key>CFBundleVersion<\/key>\s*<string>9<\/string>/);
