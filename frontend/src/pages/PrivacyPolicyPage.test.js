...
describe('Privacy Policy legal architecture', () => {
  ...
  it('omits a general 18+ restriction and describes the fixed iOS model', () => {
    ...
    expect(policySource).not.toContain('короткоживущий одноразовый код')
    expect(policySource).not.toContain('Защищённый переход в браузер')
    expect(policySource).not.toContain('не регистрирует и не передаёт серверу push/device token')
    expect(policySource).toContain('может регистрировать технический push/device token')
    expect(policySource).toContain('для доставки сервисных уведомлений')
    expect(agreementSource).not.toContain('20 активных будущих записей')
    expect(agreementSource).not.toContain('инструкций по приобретению цифрового доступа')
    expect(agreementSource).not.toContain('Переход из iOS в браузер')
    expect(agreementSource).toContain('указанных непосредственно в веб-версии Сервиса')
    expect(agreementSource).not.toContain('порядок оплаты определяются Администрацией и указываются на сайте')
    expect(agreementSource).not.toContain('оплатите тариф')
  })

  it('uses the approved revision date in changed legal documents', () => {
    expect(policySource).toContain('Дата вступления в силу: 23 сентября 2026 г.')
    expect(agreementSource).toContain('Дата последнего обновления: 23 сентября 2026 г.')
  })
})
