...
const AdminBlog = () => {
  ...
  /** Сброс экземпляра TipTap после импорта HTML (редактор не подхватывает смену content снаружи). */
  ...
  const formHasDraftContent = () => {
    const f = formData;
    return !!(
      (f.title && f.title.trim()) ||
      (f.content && f.content.trim()) ||
      (f.slug && f.slug.trim()) ||
      (f.subtitle && f.subtitle.trim()) ||
      (f.excerpt && f.excerpt.trim())
    );
  };

  const applyImportPreview = (data) => {
    ...
    setFormData((prev) => ({
      ...
      cover_image_alt: pf.cover_image_alt ?? '',
      tags: Array.isArray(pf.tags) ? pf.tags : [],
      meta_title: pf.meta_title ?? '',
      meta_description: pf.meta_description ?? '',
      canonical_url: pf.canonical_url ?? '',
      robots_noindex: !!pf.robots_noindex,
      robots_nofollow: !!pf.robots_nofollow,
      og_title: pf.og_title ?? '',
      og_description: pf.og_description ?? '',
      og_image: pf.og_image ?? '',
      twitter_title: pf.twitter_title ?? '',
      twitter_description: pf.twitter_description ?? '',
      twitter_image: pf.twitter_image ?? '',
      json_ld: pf.json_ld ?? null,
      status: pf.status || 'draft',
      scheduled_at: pf.scheduled_at ?? '',
      content: data.content_html || '',
    }));
    setSlugManuallyEdited(!!(pf.slug && String(pf.slug).trim()));
    setNewTag('');
  ...
  const handleBlogImportFile = async (e) => {
    const file = e.target.files?.[0];
    e.target.value = '';
    if (!file) return;
    const name = (file.name || '').toLowerCase();
    if (!name.endsWith('.md') && !name.endsWith('.markdown')) {
      setImportErrors(['Выберите файл с расширением .md или .markdown']);
      setImportWarnings([]);
      return;
    }
    ...
    try {
      ...
      fd.append('file', file);
      const response = await fetch(`${API_BASE_URL}/api/admin/blog/import/preview`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
        },
      ...
      if (showModal && formHasDraftContent()) {
        if (!window.confirm('Заменить текущие данные в форме импортированным файлом?')) {
          return;
        }
      }
      if (!showModal) {
      ...
      applyImportPreview(data);
    } catch (err) {
      setImportErrors([err.message || 'Ошибка сети при импорте']);
    } finally {
  ...
  const handleCoverUpload = async (e) => {
    ...
    if (!file.type?.startsWith('image/')) {
      setCoverUploadError('Можно загрузить только изображение (JPG/PNG/WEBP/GIF).');
      return;
    }
    ...
    try {
      const fd = new FormData();
      fd.append('file', file);
      const response = await fetch(`${API_BASE_URL}/api/admin/blog/upload-cover`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
        },
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(data.detail || `Ошибка загрузки (${response.status})`);
      }
      setFormData((prev) => ({
        ...prev,
        cover_image: data.url || data.relative_url || prev.cover_image,
      }));
    } catch (err) {
      setCoverUploadError(err.message || 'Не удалось загрузить изображение.');
    } finally {
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="hidden"
            onChange={handleBlogImportFile}
          />
          <button
            type="button"
            disabled={importLoading}
            onClick={() => fileImportRef.current?.click()}
            className="bg-white border border-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-50 flex items-center gap-2 disabled:opacity-50"
          >
          ...
          <button
            ...
            className="bg-[#4CAF50] text-white px-4 py-2 rounded-lg hover:bg-[#43a047] flex items-center gap-2"
          >
      ...
      {importErrors.length > 0 && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm">
          <p className="font-medium mb-1">Ошибка импорта</p>
      ...
      {importWarnings.length > 0 && importErrors.length === 0 && (
        <div className="mb-4 p-4 bg-amber-50 border border-amber-200 text-amber-900 rounded-lg text-sm">
          <p className="font-medium mb-1">Импорт выполнен, замечания</p>
          <ul className="list-disc list-inside space-y-0.5">
            ...
            <div className="p-6">
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  disabled={importLoading}
                  onClick={() => fileImportRef.current?.click()}
                  className="text-sm px-3 py-1.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 disabled:opacity-50"
                >
                ...
                {activeTab === 'content' && (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4CAF50]"
                          required
                        />
                      </div>
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex flex-wrap gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg bg-[#4CAF50] text-white hover:bg-[#43a047] disabled:opacity-50"
                            >
                              {coverUploadLoading ? 'Загрузка…' : 'Загрузить изображение'}
                            </button>
                          ...
                          {formData.cover_image && (
                            <img
                              ...
                              alt={formData.cover_image_alt || 'Обложка статьи'}
                              className="mt-1 h-24 w-40 rounded-lg object-cover border border-gray-200"
                            />
                          )}
                        </div>
                      </div>
                      ...
                      <div className="mb-3">
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4CAF50]"
                          />
                          <button
                            type="button"
                            onClick={() => {
                    ...
                    <div>
                      ...
                      <BlogEditor
                        key={`blog-editor-${editingPost?.id ?? 'new'}-${editorSessionKey}`}
                ...
                <div className="flex justify-between gap-4 pt-6 border-t mt-6">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 text-[#4CAF50] bg-green-50 rounded-lg hover:bg-green-100"
                  ...
                  <div className="flex gap-4">
                    ...
                    <button
                      type="submit"
                      className="px-4 py-2 bg-[#4CAF50] text-white rounded-lg hover:bg-[#43a047] disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={submitLoading}
                    >
                      {submitLoading ? (
