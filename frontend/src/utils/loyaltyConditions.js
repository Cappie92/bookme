 ...
 * @param {object} typeMap - Маппинг UI type → backend condition_type
 * @returns {object} - {condition_type, parameters}
 */
function _convertUIFormatToBackend(uiCondition, typeMap) {
 ...
 * @param {string} conditionType - Backend condition_type
 * @returns {boolean}
 */
export function isConditionTypeSupported(conditionType) {
  return SUPPORTED_CONDITION_TYPES.includes(conditionType)
}

/**
 * Получает список поддерживаемых типов для UI (с маппингом UI → backend)
 * @returns {Array<{uiType: string, backendType: string, label: string}>}
 */
export function getSupportedConditionTypesForUI() {
  // Маппинг UI type → backend type → label
  const UI_TYPE_MAP = {
    "first_visit": { backendType: "first_visit", label: "Первая запись" },
    "returning_client": { backendType: "returning_client", label: "Возвращение клиента" },
    "regular_visits": { backendType: "regular_visits", label: "Регулярные визиты" },
    "birthday": { backendType: "birthday", label: "День рождения" },
    "happy_hours": { backendType: "happy_hours", label: "Счастливые часы" },
    "service_discount": { backendType: "service_discount", label: "Скидка на услуги" },
  }

  return Object.entries(UI_TYPE_MAP).map(([uiType, { backendType, label }]) => ({
    uiType,
    backendType,
    label,
  }))
...
const PERIOD_TO_DAYS = { week: 7, month: 30, year: 365 }
 ...
 * Совпадает с backend utils/loyalty_params. Принимает старый и новый форматы.
 */
export function normalizeParametersForComparison(conditionType, parameters, ruleDiscountPercent = null) {
  ...
  if (conditionType === 'returning_client') {
    ...
    let maxDays = p.max_days_since_last_visit
    if (maxDays != null && typeof maxDays === 'number') maxDays = Math.floor(maxDays)
    else maxDays = null
    const out = { min_days_since_last_visit: Math.max(0, minDays) }
  ...
  if (conditionType === 'happy_hours') {
    let days = p.days ?? p.days_of_week
    if (Array.isArray(days)) {
      days = _sortNum([...new Set(days.filter((d) => typeof d === 'number' && d >= 1 && d <= 7))])
    } else {
      days = [1, 2, 3, 4, 5]
    }
 ...
 * Единая логика с mobile. Используется для «Активен» / поиска скидки по шаблону.
 */
export function templateMatchesDiscount(template, discount) {
  if (!discount.is_active || discount.discount_type !== 'quick') return false
  if (!template.conditions?.condition_type) return false
  const tc = template.conditions
  const dc = discount.conditions
  let discountCt
  let discountParams = {}
  if (dc && typeof dc === 'object' && !Array.isArray(dc)) {
    discountCt = dc.condition_type
    discountParams = dc.parameters ?? {}
  }
  ...
  if (tc.condition_type === 'service_discount' && normT._invalid === true && normD._invalid !== true) return true
  return stableStringify(normT) === stableStringify(normD)
}

/**
 * Активная скидка по шаблону (для деактивации). Та же логика, что в mobile `findActiveDiscountForTemplate`.
 */
export function findActiveDiscountForTemplate(template, discounts) {
  if (!Array.isArray(discounts)) return null
  const found = discounts.find((d) => templateMatchesDiscount(template, d))
  return found ?? null
}

/** Типы quick-правил: не более одной записи в БД на мастера (см. backend BINARY_QUICK_CONDITION_TYPES). */
export const BINARY_QUICK_CONDITION_TYPES = new Set(['first_visit', 'birthday'])

export function getQuickConditionType(discount) {
  const c = discount?.conditions
  if (c && typeof c === 'object' && !Array.isArray(c)) return c.condition_type || null
  return null
}

/** Все quick-скидки мастера с данным condition_type (включая неактивные). */
export function quickDiscountsByConditionType(discounts, conditionType) {
  if (!Array.isArray(discounts) || !conditionType) return []
  return discounts.filter((d) => d.discount_type === 'quick' && getQuickConditionType(d) === conditionType)
}

export function isBinaryQuickConditionType(ct) {
  return BINARY_QUICK_CONDITION_TYPES.has(ct)
}

/**
 * Одна каноническая запись «Новый клиент» для UI при legacy-дубликатах в БД:
 * приоритет активной, иначе запись с максимальным id.
 * Остальные строки first_visit в списке не показываются (данные в БД не трогаем).
 */
export function canonicalFirstVisitQuickRule(discounts) {
