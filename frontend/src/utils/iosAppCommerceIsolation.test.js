...
describe('ios_app operational web editor isolation', () => {
  ...
  it('derives ios_app only from the server-provided session claim', () => {
    ...
    expect(auth).toContain('const origin = userData.web_session_origin || null')
    expect(auth).toContain("const isIosAppWebSession = webSessionOrigin === 'ios_app'")
  ...
  it('keeps ios_app hidden-module and commerce API fetches disabled', () => {
    ...
    expect(dashboard).toContain('useMasterSubscription({ enabled: !isIosAppWebSession })')
  ...
  it('keeps commerce navigation hidden in the ios_app shell', () => {
    ...
    expect(header).toContain('{!isIosAppWebSession && (')
