...
export default function ClientDashboardStats() {
  ...
  const loadStats = async () => {
    try {
      setLoading(true);
      const data = await apiGet('/api/client/dashboard/stats');
      setStats(data);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {/* Основная статистика - вертикальный стек */}
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <p className="text-3xl font-bold text-gray-900">{stats && stats.past_bookings ? stats.past_bookings : 0}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <p className="ml-4 text-base font-medium text-gray-900">Будущих записей</p>
            </div>
            <p className="text-3xl font-bold text-gray-900">{stats && stats.future_bookings ? stats.future_bookings : 0}</p>
          </div>
        </div>

        {/* Любимых салонов - показывается только если salonsEnabled=true */}
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <p className="ml-4 text-base font-medium text-gray-900">Любимых салонов</p>
              </div>
              <p className="text-3xl font-bold text-gray-900">{stats.top_salons.length}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <p className="ml-4 text-base font-medium text-gray-900">Любимых мастеров</p>
            </div>
            <p className="text-3xl font-bold text-gray-900">{stats && stats.top_masters ? stats.top_masters.length : 0}</p>
          </div>
        </div>
      </div>

      {/* Топ салонов - показывается только если salonsEnabled=true */}
      {salonsEnabled && stats && stats.top_salons && stats.top_salons.length > 0 && (
