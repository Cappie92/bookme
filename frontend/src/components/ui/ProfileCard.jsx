import React from 'react'

const ProfileCard = ({ 
  name, 
  specialty, 
  rating, 
  reviews, 
  image, 
  className = '', 
  ...props 
}) => {
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-neutral-200 p-4 ${className}`} {...props}>
      <div className="flex items-center relative h-16">
        <div className="flex items-center flex-1">
          <div className="w-16 h-16 rounded-full overflow-hidden bg-neutral-200 mr-3">
            {image ? (
              <img src={image} alt={name} className="w-full h-full object-cover" />
            ) : (
                          <div className="w-full h-full bg-[#E8F5E8] flex items-center justify-center">
              <span className="text-[#4CAF50] font-semibold text-xl">
                {name?.charAt(0) || 'U'}
              </span>
            </div>
            )}
          </div>
          <div className="flex flex-col justify-center">
            <h3 className="text-sm font-medium text-neutral-900">{name}</h3>
            <p className="text-xs text-neutral-600">{specialty}</p>
          </div>
        </div>
        <div className="absolute bottom-2 right-2 flex items-center space-x-1">
          <span className="text-sm font-medium text-neutral-900">{rating}</span>
          <svg
            className="w-3 h-3 text-[#4CAF50]"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
          <span className="text-xs text-neutral-600">({reviews})</span>
        </div>
      </div>
    </div>
  )
}

export default ProfileCard 