...
const Calendar = ({
  ...
  availableDates = [],
  className = '',
  /** YYYY-MM-DD или Date — месяц сетки при открытии / после смены выбранной даты */
  initialMonthAnchor = null,
}) => {
  const [currentMonth, setCurrentMonth] = useState(() => {
    const anchor = parseToLocalDate(initialMonthAnchor) || parseToLocalDate(selectedDate)
    if (anchor) return new Date(anchor.getFullYear(), anchor.getMonth(), 1)
    const now = new Date()
    return new Date(now.getFullYear(), now.getMonth(), 1)
  })

  const [selectedDateState, setSelectedDateState] = useState(() => parseToLocalDate(selectedDate))

  useEffect(() => {
    setSelectedDateState(parseToLocalDate(selectedDate))
  }, [selectedDate])

  useEffect(() => {
    const anchor = parseToLocalDate(initialMonthAnchor)
    if (anchor) {
