...
export default function PastAppointments() {
  ...
  const handleApplyStatus = async (appointmentId, fallbackStatus) => {
    ...
    try {
      ...
      console.error('[API] Ошибка при обновлении статуса:', url, status, detail, e);
      if (status === 403) {
        alert(detail || 'Нельзя изменять статус записи, отмененной клиентом');
      } else if (status === 404) {
        alert(detail || 'Запись не найдена');
      } else {
        alert(detail || 'Ошибка при обновлении статуса');
      }
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#4CAF50] focus:border-[#4CAF50]"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div>
            ...
            >
              ...
              <option value="awaiting_confirmation">На подтверждении</option>
          ...
          <div className="flex items-end gap-2">
            <button
              ...
              className="px-4 py-2 bg-[#4CAF50] text-white rounded-lg hover:bg-[#43a047] transition-colors focus:outline-none focus:ring-2 focus:ring-[#4CAF50] focus:ring-offset-2"
            >
              Применить
            </button>
            <button
                    ...
                    <tr key={appointment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <span className="flex items-center gap-1">
                          {(appointment.client_display_name || appointment.client_name) || 'Клиент'}
                          {appointment.client_phone && <span className="text-xs text-gray-500 ml-1">{appointment.client_phone}</span>}
                          {appointment.has_client_note ? (
                            <span title={appointment.client_note || ''} className="text-gray-900 cursor-help no-underline">
                          ...
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2 justify-end">
                              ...
                              <button
                                ...
                                className="px-3 py-1 bg-[#4CAF50] text-white rounded hover:bg-[#43a047] transition-colors focus:outline-none focus:ring-2 focus:ring-[#4CAF50] focus:ring-offset-1 disabled:opacity-50"
                              >
                                Сохранить
                              </button>
                            </div>
