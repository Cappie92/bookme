# Вердикт: `/api/master/services/public` endpoint
...
- ✅ Найден `/api/salon/services/public` в `backend/routers/salon.py:208` (для салонов)
...
**Вариант 1: Оставить в PUBLIC_ENDPOINTS (рекомендуется)**
- Endpoint используется во frontend для публичного сценария
- Если endpoint будет реализован в backend, он не будет блокироваться auth-guard'ом
- Если endpoint не существует, запрос вернёт 404, что корректно обрабатывается в `catch` блоке
- **Безопасно:** даже если endpoint появится без auth, он не будет блокироваться

**Вариант 2: Убрать из PUBLIC_ENDPOINTS и исправить frontend**
...
**Причины:**
1. Frontend уже использует этот endpoint
2. Если endpoint будет реализован, он должен быть публичным (для публичной страницы бронирования)
3. Если endpoint не существует, запрос вернёт 404, что корректно обрабатывается
4. Минимальные изменения в коде

### Финальное решение

**Оставить `/api/master/services/public` в `PUBLIC_ENDPOINTS`** с комментарием:
