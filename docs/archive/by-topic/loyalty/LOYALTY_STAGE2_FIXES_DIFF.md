...
**Дата:** 2026-01-21  
**Цель:** Исправить auth-gating и обработку ошибок для полного parity с MOBILE
...
@@ -25,7 +25,30 @@ export const apiRequest = async (endpoint, options = {}) => {
   try {
...
1. **Auth-gating:** Все компоненты теперь проверяют `authLoading` и `isAuthenticated` перед запросами
2. **Обработка ошибок:** Добавлена обработка 401, 409, 404 во всех компонентах
3. **Форма настроек:** Уже полная (все поля есть), показываются условно (как в MOBILE)
