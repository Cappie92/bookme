...
graph TB
    ...
    %% Внешние системы
    Yandex[🗺️ Яндекс Карты<br/>Обычная ссылка поиска]
    ...
    Client -->|Открывает ссылку<br/>по строковому адресу| Yandex
...
Адрес сохраняется строкой. При нажатии пользователь открывает `https://yandex.ru/maps/?text=...`. API-ключи Maps, Geocoder и Suggest не требуются. OAuth и Метрика — отдельные интеграции.
...
Мастер вводит адрес → backend сохраняет строку → публичная страница возвращает ссылку на поиск в Яндекс Картах → пользователь открывает её. Геокодирование и координаты не используются.
...
- ❌ Открытие адреса в Яндекс Картах по обычной HTTPS-ссылке
...
- **Пользователь → Яндекс Карты:** обычная HTTPS-ссылка без ключа
